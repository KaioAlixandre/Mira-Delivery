...
const AddProductModal: React.FC<Props> = ({ categories, onClose, onAdd, onManageCategories }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'file' && e.target instanceof HTMLInputElement) {
      ...
      if (form.images.length + files.length > 5) {
        notify('Você pode adicionar no máximo 5 imagens', 'warning');
        return;
      }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl my-8 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center rounded-t-2xl shrink-0">
          <div>
            <h3 className="text-xl font-bold text-slate-800">Adicionar Novo Produto</h3>
            <p className="text-xs text-slate-500 mt-0.5">Preencha as informações do produto</p>
          </div>
          <button 
            onClick={onClose}
                  ...
                  <div className="flex flex-col gap-2 p-3 bg-purple-50 rounded-lg">
                    ...
                    {form.receiveComplements && (
                      <div className="flex items-center gap-2 mt-1">
                        <label htmlFor="quantidadeComplementos" className="text-xs font-medium text-slate-700">Qtd. permitida:</label>
                        <input
                          type="number"
