...
import { X, Plus, Pencil, Trash2, Save, Layers, AlertCircle } from 'lucide-react';
...
const ModalGerenciarCategoriasAdicionais: React.FC<Props> = ({ categories: initialCategories, onClose, onCategoriesChange }) => {
  const [categories, setCategories] = useState<AdditionalCategory[]>(initialCategories);
  ...
  const handleStartEdit = (category: AdditionalCategory) => {
  ...
  const handleSaveEdit = async (id: number) => {
    ...
    try {
      setLoading(true);
      setError('');
      const updatedCategory = await apiService.updateAdditionalCategory(id, editName.trim());
  ...
  const handleDelete = async (id: number) => {
    ...
    const hasAdditionals = category?.additionalsCount && category.additionalsCount > 0;

    if (hasAdditionals) {
      alert(`Não é possível excluir esta categoria. Ela possui ${category?.additionalsCount} adicional(is) associado(s).`);
      return;
    }
    ...
    try {
      setLoading(true);
      setError('');
      await apiService.deleteAdditionalCategory(id);
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError('');
      const newCategory = await apiService.createAdditionalCategory(newCategoryName.trim());
        ...
        <div className="flex items-center justify-between gap-4 p-5 sm:p-6 border-b border-slate-200 bg-gradient-to-br from-slate-50 via-white to-slate-50/80">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-lg sm:text-xl font-bold text-slate-900 tracking-tight">Categorias de Adicionais</h3>
              <p className="text-xs text-slate-500 mt-0.5">Crie, edite e remova categorias de adicionais</p>
            </div>
          </div>
          <button
            type="button"
          ...
          <form onSubmit={handleAddCategory} className="mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                placeholder="Ex: Extras, Acompanhamentos..."
                      ...
                      <>
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0 group-hover:bg-brand-light transition-colors">
                            <Layers className="w-4 h-4 text-slate-500 group-hover:text-brand transition-colors" />
                          </div>
                          <div className="min-w-0">
...
export default ModalGerenciarCategoriasAdicionais;
